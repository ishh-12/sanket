(function () {
  const canvas = document.getElementById("sanket-stage");
  ...
  pillarItems.forEach((pillar, index) => {
    pillar.addEventListener("pointerenter", () => {
      activePillar = index;
      pillarItems.forEach((item) => item.classList.toggle("is-lit", item === pillar));
    });
    pillar.addEventListener("pointerleave", () => {
      activePillar = -1;
      pillarItems.forEach((item) => item.classList.remove("is-lit"));
    });
  });

  if (window.gsap && window.ScrollTrigger) {
    window.gsap.registerPlugin(window.ScrollTrigger);
    window.gsap.utils.toArray(".chapter").forEach((chapter) => {
  ...
  graphPoints.forEach((point, index) => {
    ...
    graph.add(node);
    if (index > 0) graph.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints([graphPoints[index - 1], point]), materials.line));
  });
  root.add(graph);
  ...
  function updatePageState() {
    ...
    nav.classList.toggle("quiet", window.scrollY > window.innerHeight * 0.55);
  ...
  function scaleGroup(group, visible, size) {
    group.visible = visible;
    const target = visible ? size : 0.001;
  ...
  function render(time) {
    ...
    root.rotation.y += delta * spin;
    root.rotation.x += (pointer.y * 0.13 - root.rotation.x) * 0.03;
    root.rotation.z += (pointer.x * -0.08 - root.rotation.z) * 0.03;
    root.position.x += ((compact ? 0 : pointer.x * 0.28) - root.position.x) * 0.025;
    ...
    camera.position.x += ((activeScene === "why" ? 0.85 : 0) + pointer.x * 0.12 - camera.position.x) * 0.02;
    camera.lookAt(0, -0.05, 0);

    limeLight.position.x += (pointer.x * 3 - limeLight.position.x) * 0.04;
    limeLight.position.y += (-1.2 + pointer.y * -1.4 - limeLight.position.y) * 0.04;

    const pulse = Math.sin(time * 0.0018) * 0.025;
    core.scale.setScalar(1 + pulse + (activeScene === "restore" ? 0.08 : 0));
    inner.rotation.y -= delta * 0.0006;
    circuits.rotation.y += delta * 0.0005;
    helix.rotation.y += delta * 0.00025;

    particles.children.forEach((particle, index) => {
      const phase = particle.userData.angle + time * particle.userData.speed;
      ...
      particle.material.opacity = 0.16 + Math.sin(time * 0.0018 + index) * 0.08;
    });

    const buildOn = activeScene === "build" || activeScene === "pillars";
    scaleGroup(layers, buildOn, activeScene === "pillars" ? 1.8 : 1.35);
    layers.children.forEach((mesh, index) => {
      const spread = activeScene === "build" ? 1.35 : activeScene === "pillars" ? 2.4 : 0.45;
      const angle = mesh.userData.angle + time * 0.00022;
      ...
      mesh.rotation.y += delta * 0.0008 + index * 0.00002;
    ...
    const disturbed = activeScene === "notice" || activeScene === "why";
    helix.scale.lerp(new THREE.Vector3(disturbed ? 1.03 : 1, disturbed ? 0.93 : 1, disturbed ? 1.03 : 1), 0.045);
    materials.amber.opacity = activeScene === "notice" ? 0.9 : 0.62;

    scaleGroup(graph, activeScene === "notice" || activeScene === "why", activeScene === "why" ? 1.55 : 1);
    graph.rotation.y = activeScene === "why" ? Math.sin(time * 0.0005) * 0.35 : 0;

    scaleGroup(storage, activeScene === "storage", 1.25);
    storage.rotation.y -= delta * 0.00028;
    ...
    scaleGroup(repair, activeScene === "heal" || activeScene === "restore", activeScene === "restore" ? 1.45 : 1.1);
    repair.children.forEach((child, index) => {
      if (child.material && child.material.opacity !== undefined) child.material.opacity = activeScene === "restore" ? 0.82 : 0.44 + index * 0.04;
    });

    scaleGroup(prediction, activeScene === "predict", 1.35);
    prediction.rotation.y = Math.sin(time * 0.0005) * 0.16;

    if (activeScene === "final") {
      scaleGroup(layers, false, 1);
      scaleGroup(graph, false, 1);
      scaleGroup(storage, false, 1);
      scaleGroup(repair, false, 1);
      scaleGroup(prediction, false, 1);
